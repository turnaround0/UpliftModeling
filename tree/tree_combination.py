...
def choose_attr(df, attributes, predict_attr, treatment_attr,
    ...
    kbest = SelectKBest(k=2).fit(df[attributes], df[predict_attr])
    selected = kbest.get_support(indices=True)
    best_attr = np.asarray(attributes)[selected]
...
def build_tree(df, cols, predict_attr='Y', treatment_attr='T',
    ...
    if best_attr is None:
        ...
        sub_1 = df[(df[best_attr[0]] <= threshold[0]) & (df[best_attr[1]] <= threshold[1])]
        # sub_2 = df[df[best_attr] > threshold]
        sub_2 = df[(df[best_attr[0]] > threshold[0]) & (df[best_attr[1]] > threshold[1])]
...
def predict(node, row_df):
    ...
    if row_df[node.attr[0]] <= node.thres[0] and row_df[node.attr[1]] <= node.thres[1]:
        return predict(node.left, row_df)
    # elif row_df[node.attr[0]] > node.thres[0] and row_df[node.attr[1]] > node.thres[1]:
